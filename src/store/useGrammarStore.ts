...
export const useGrammarStore = create<GrammarStore>((set, get) => ({
  ...
  checkGrammar: async (text: string) => {
    ...
    try {
      ...
      // Ollama returns data directly, others use choices
      const aiResponse = selectedModel === "ollama" 
        ? (data as any).message?.content || data 
        : data.choices[0]?.message?.content;

      try {
        const grammarErrors = typeof aiResponse === 'string' 
