...
const addActivityDate = (dates?: string[]): string[] => {
  ...
  persist<ResumeStore>(
    (set, get) => ({
      ...
      updateResumeFromFile: (resume, sourceModifiedAt) => {
        ...
        if (
          !shouldImportResumeFromFile(resume, localResume, sourceModifiedAt)
        ) {
          return false;
        }

        const importedResume = normalizeImportedResume(
          resume,
          sourceModifiedAt,
        );
      ...
      deleteResume: (resume) => {
        ...
        set((state) => {
          // 创建一个完全新的对象，确保 Zustand 能检测到变化
          const newResumes: Record<string, ResumeData> = {};

          // 复制除了要删除的简历之外的所有简历
          Object.keys(state.resumes).forEach((id) => {
            if (id !== resumeId) {
              newResumes[id] = state.resumes[id];
            }
          });
          ...
          const newState = {
            ...
            activeResume: null, // 总是重置 activeResume，避免引用问题
          };

          // 如果删除的不是当前激活的简历，则保留原激活状态
          if (state.activeResumeId !== resumeId) {
            newState.activeResume = state.activeResume;
          }
        ...
        (async () => {
          try {
            ...
            try {
              ...
              console.log(
                "Failed to delete file, but resume will still be deleted from store",
              );
            }
          } catch (error) {
