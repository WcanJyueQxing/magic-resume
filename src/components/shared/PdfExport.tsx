...
import { Download, Loader2, ChevronDown, ShieldCheck, CheckCircle2, XCircle } from "lucide-react";
...
const PdfExport = ({ children }: { children?: React.ReactNode }) => {
  ...
  const handleExport = async () => {
    await exportToPdf({
      ...
      onSuccess: () => setExportResult({ type: "success", message: t("toast.success") }),
      onError: () => setExportResult({ type: "error", message: t("toast.error") })
    });
  };
  ...
  const handleJsonExport = () => {
    exportResumeAsJson({
      ...
      onSuccess: () => setExportResult({ type: "success", message: t("toast.jsonSuccess") }),
      onError: () => setExportResult({ type: "error", message: t("toast.jsonError") })
    });
  };
  ...
  const handleMarkdownExport = () => {
    exportResumeAsMarkdown({
      ...
      onSuccess: () => setExportResult({ type: "success", message: t("toast.markdownSuccess") }),
      onError: () => setExportResult({ type: "error", message: t("toast.markdownError") }),
      markdownOptions: {
        basicFieldLabels: {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      setExportResult({ type: "success", message: t("toast.success") });
    } catch (error) {
      console.error("Print error:", error);
      setExportResult({ type: "error", message: t("toast.error") });
    } finally {
      setIsPrinting(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* 导出结果对话框 */}
      <Dialog open={!!exportResult} onOpenChange={(val) => {
        ...
        >
          <DialogHeader className="items-center gap-4 py-4">
            ...
            <DialogTitle className="text-xl font-bold text-center">
              {exportResult?.type === "success" ? t("result.successTitle") : t("result.errorTitle")}
            </DialogTitle>
            <DialogDescription className="text-center text-muted-foreground">
              {exportResult?.message}
            </DialogDescription>
          </DialogHeader>
          <p className="text-center text-sm text-muted-foreground/60 pb-2">
            {t("result.clickToClose")}
          </p>
        </DialogContent>
      </Dialog>
    </>
